...
async fn run_broker(listener: UnixListener, memfd: OwnedFd, stop: CancellationToken) {
    ...
    loop {
        let client = tokio::select! {
            ...
            _result = sends.join_next(), if !sends.is_empty() => continue,
            client = listener.accept() => match client {
                Ok((client, _address)) => client,
                Err(error) => {
        ...
        // The owner cancels the token before its drop returns, so a request
        // arriving after the owner is gone is refused deterministically even
        // if this task has not observed the cancellation in `select!` yet.
        ...
        sends.spawn(async move {
            let result = tokio::select! {
                biased;
                () = stop.cancelled() => return,
                result = AsyncFdPassingExt::send_fd(&client, memfd.as_raw_fd()) => result,
            };
