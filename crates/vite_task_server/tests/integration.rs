...
#[cfg(unix)]
fn connect_raw(name: &OsStr) -> Stream {
    use interprocess::local_socket::{GenericFilePath, ToFsName};
    let resolved = name.to_fs_name::<GenericFilePath>().expect("resolve socket name");
    Stream::connect(resolved).expect("connect raw")
}

#[cfg(windows)]
fn connect_raw(name: &OsStr) -> Stream {
    use interprocess::local_socket::{GenericNamespaced, ToNsName};
    let resolved = name.to_ns_name::<GenericNamespaced>().expect("resolve pipe name");
    Stream::connect(resolved).expect("connect raw")
}

fn send_frame(stream: &mut Stream, request: &Request<'_>) {
